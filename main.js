...
const cancel = document.getElementById("cancelOrd");
...
function goToCar() {
  ...
  prods.innerHTML = "";
  numCar.innerHTML =
    carrito.length === 1 ? carrito.length + " item" : carrito.length + " items";
  if (carrito.length > 0) {
    ...
    let tot = document.createElement("strong");
    tot.className = "col-9";
    ...
    let emptcol = document.createElement("div");
    emptcol.className = "col-12";
    let empt = document.createElement("h3");
    empt.style = "text-align: center;";
    empt.innerHTML = "El carrito se encuentra vacio";
    emptcol.appendChild(empt);
    prods.appendChild(emptcol);
  }
}
...
function remCar(event) {
  let idcom = parseInt(event.srcElement.id.replace("r", ""));
